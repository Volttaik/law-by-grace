...
import { extractImages, parseSections, resolveArticleBanner } from "@/lib/article-content";
...
export async function GET(req: Request) {
  ...
  const articles = await prisma.article.findMany({
    ...
    include: {
      ...
      referenceCourse: {
        select: { id: true, title: true, slug: true, banner: true, courseCode: true, department: true, university: true },
      },
  ...
  return NextResponse.json(articles.map(a => {
    ...
    return {
      ...
      hasDraft: a.draft !== null,
      status: a.isPublished && a._count.editions > 0 ? "published" : "draft",
      views: a.views,
      likes: a.likes,
      ...
      referenceCourse: a.referenceCourse,
      author: a.author,
